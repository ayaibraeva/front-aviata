  ...
  <div class="card">
    <div class="card__left">
      ...
      <div class="card__left-bottom">
        <BaseButton variant="blue" class="detail-btn">{{
          isMobile ? "Детали" : "Детали перелета"
        }}</BaseButton>
        <BaseButton variant="blue" class="center-btn">
          Условия тарифа</BaseButton
        >
...
export default {
  ...
  computed: {
    ...
    destinationInfo() {
      return this.card.itineraries[0][0].segments[
        this.card.itineraries[0][0].segments.length - 1
      ];
    },
    getSegments() {
      ...
      if (segments.length === 1) {
        ...
        return `${
          this.isMobile
            ? `${this.card.itineraries[0][0].stops} пересадки`
            : `через ${segments[0]["airport_dest"]}, ${this.getStopsTime}`
        }`;
      } else {
    ...
    getDepDate() {
      let info = this.card.itineraries[0][0].segments[0]["dep_time"]
        .substring(0, 10)
        .toLowerCase();
      return info;
    },
    getArrDate() {
      let info = this.destinationInfo["arr_time"]
        .substring(0, 10)
        .toLowerCase();
      return info;
    },
    ...
    getTraveltime() {
      let travelHour = Math.floor(
        this.card.itineraries[0][0].traveltime / 3600
      );
      let travelMinutes = Math.floor(
        (this.card.itineraries[0][0].traveltime / 60) % 60
      );
